...
test("canRegister : refuse moins de 15 ans, accepte à partir de 15 ans pile", () => {
  assert.equal(canRegister(d('2011-09-11'), d('2026-09-10')), false);
  assert.equal(canRegister(d('2011-09-10'), d('2026-09-10')), true);
});

test('canRegister : refuse une date de naissance dans le futur', () => {
  assert.equal(canRegister(d('2030-01-01'), d('2026-09-10')), false);
});
